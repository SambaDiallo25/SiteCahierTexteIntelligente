...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (compteurs.length > 0) {
    const observateurCompteur = new IntersectionObserver(function (entrees) {
      entrees.forEach(function (entree) {
        if (entree.isIntersecting) {
  ...
  const cartesFonctionnalites = document.querySelectorAll('.carte-fonctionnalite');
    ...
    boutonsFiltres.forEach(function (btn) {
      btn.addEventListener('click', function () {
        ...
        cartesFonctionnalites.forEach(function (carte) {
          if (categorie === 'tout' || carte.getAttribute('data-categorie') === categorie) {
  ...
  if (formulaire) {
    formulaire.addEventListener('submit', function (e) {
      ...
      if (!nom.value.trim() || nom.value.trim().length < 2) {
        afficherErreur(nom, 'erreur-nom', 'Veuillez entrer votre nom (au moins 2 caractères)');
        estValide = false;
      } else {
      ...
      if (!email.value.trim() || !regexEmail.test(email.value)) {
        afficherErreur(email, 'erreur-email', 'Veuillez entrer une adresse email valide');
        estValide = false;
      } else {
      ...
      if (!message.value.trim() || message.value.trim().length < 10) {
        afficherErreur(message, 'erreur-message', 'Votre message doit contenir au moins 10 caractères');
        estValide = false;
      } else {
